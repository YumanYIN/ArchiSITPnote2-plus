...
public class PostDAO extends BaseDAO {
    ...
    public List<Post> getPublicPost(int profileId){
        String sql = "SELECT * FROM post WHERE ((`authorid` = ?) OR (`authorid` <> ? AND `typevisible` = everyone))";
        Object param[] = {
                profileId,
                profileId
        };
    ...
    private List<Post> turnToPost(List<Map<String, Object>> result){
        ...
        for (Map<String, Object> postMap:result) {
            ...
            post.setAuthor(profileDAO.getProfileById(Integer.valueOf(postMap.get("authorid").toString())));
